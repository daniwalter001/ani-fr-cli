...
class Vostfree:
    ...
    api = "https://vostfree.ws"

    script_defaut_v1 = "https://anime-sama.fr/js/defaut/script_defaut_v1.js"
    script_oeuvre = "https://anime-sama.fr/js/contenu/script_oeuvre.js?filever=6624"
    script_episode = "/episodes.js?filever=5306"

    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/237.84.2.178 Safari/537.36",
        "referer": "https://vostfree.ws",
        "Cookie": "PHPSESSID=rc565777jc2sduesf8tckqqmbg",
    ...
    def search(self, query="", type=""):
        ...
        soup = self.souper.postAndParse(api, payload=payload, extra_headers=_headers)
    ...
    def fetch(self, query="", type="TV", extra_titles: list = []):
        ...
        first_try = check_anime(queries, search_results or [])
    ...
    def extract_search_results(self, soup: BeautifulSoup):
        ...
        res_nodes = soup.select("div.search-result", limit=30)
        ...
        for i, node in enumerate(res_nodes):
            ...
            to_append = {
                "title": self.get_first_from_selects(node, ["div.title a"]).text,
                "url": urllib.parse.urljoin(
                    self.url,
                    self.get_first_from_selects(node, ["div.title a"]).get("href"),  # type: ignore
                ),
                "image": self.get_first_from_selects(node, ["span.image img"]).get(
                    "src"
                ),
                "description": remove_special_chars(
                    self.get_first_from_selects(node, ["div.desc"]).text.replace(
                        "\n", " "
                    )
                ),
                "duration": self.get_first_from_selects(
                    node, ["div.alt div.year7 b"]
                ).text,
                "detail": remove_special_chars(
                    self.get_first_from_selects(node, ["ul.additional"]).text
                ),
                "type": None,
            }
    ...
    def fetch_eps(self, season_url: str):
        ...
        try:
            api = urllib.parse.urljoin(self.url, season_url + self.script_episode)
