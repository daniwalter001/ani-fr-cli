...
from util.functions import check_anime, extract_links, join_path


class AnimeSama:
    url = "https://anime-sama.fr"
    api = "https://anime-sama.fr"
    ...
    def search(self, query="", type=""):
        ...
        api = urllib.parse.urljoin(self.url, "/template-php/defaut/fetch.php")
        payload = {
            "query": query,
        }

        soup = self.souper.postAndParse(
            api, payload=payload, extra_headers=self.headers
        )
    ...
    def extract_search_results(self, soup: BeautifulSoup):
        ...
        res_nodes = soup.select("a", limit=30)
        ...
        for i, node in enumerate(res_nodes):
            ...
            to_append = {
                "title": self.get_first_from_selects(node, ["div h3"]).text,
                "url": urllib.parse.urljoin(
                    self.url,
                    node.get("href"),  # type: ignore
                ),
                "image": self.get_first_from_selects(node, ["img"]).get("src"),
